...
public class AddLocalEntry extends ServiceObligationProcessorBase {
  ...
  public void processObligation(EventProcessingObligation obligation,
	...
	if (event.getRemoteObj().getObjectType() == IdObjectType.PERSON) {
	  User user = idDao.addUser(event);	  
	  audit(user, null, LogDefType.sync_agent_created_user, obligation, event, cb, null);
	} else if (event.getRemoteObj().getObjectType() == IdObjectType.GROUP) {
	  log.warn("Add not supported for Groups. ");
	  throw new ObligationProcessingException("Add not supported for Groups");
	} else {
