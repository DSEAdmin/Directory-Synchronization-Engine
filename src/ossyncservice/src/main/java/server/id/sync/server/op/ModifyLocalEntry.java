...
public class ModifyLocalEntry extends ServiceObligationProcessorBase {
  ...
  public void processObligation(EventProcessingObligation obligation,
	...
	if (event.getRemoteObj().getObjectType() == IdObjectType.PERSON) {
	  User user = idDao.updateUser(event);	  
	  audit(user, null, LogDefType.sync_agent_modified_user, obligation, event, cb, null);
	} else {
